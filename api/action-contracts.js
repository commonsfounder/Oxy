...
const ACTION_CONTRACTS = {
  ...
  set_notification_preference: {
    ...
    paramHints: {
      ...
      category: 'watch | digest | delivery | reply_needed | occasion | commitment | action_required — omit to set the default for everything',
  ...
  create_scheduled_task: {
    ...
    optional: ['recurrence', 'time', 'day_of_week', 'date', 'due_date', 'condition', 'interval_minutes', 'expires_at', 'budget_cap', 'watch_type', 'threshold', 'comparator', 'notify_rule', 'source_url', 'target_state', 'context_event', 'context_metric', 'context_radius_metres'],
    paramHints: {
      ...
      context_event: 'arrive_home | leave_home — only for the current user, using their saved home and fresh device location',
      context_metric: 'latest_heart_rate | resting_heart_rate | steps_today | sleep_minutes_last_night — only when the user explicitly names a threshold',
      context_radius_metres: 'optional home radius in metres; omit for the safe default unless the user explicitly asks for a wider or narrower boundary'
    },
    ...
    guidance: 'Create durable background work only when the user explicitly asks to remind, schedule, watch, monitor, or check again. A condition such as "when the price drops" becomes a bounded poll with an expiry. Never invent a schedule or silently turn an ordinary task into recurring work. ' +
      ...
      'For a contextual reminder about the current user ("remind me to take the parcel when I get home", "tell me about the bins when I leave home"), set context_event to arrive_home or leave_home and make instruction the actual reminder to deliver. This uses a fresh device location and the user\'s saved home, records the current inside/outside state as a baseline, and fires only on a real transition. Never use it for another household member — no shared-presence source exists yet — and never imply it will work when location reminders are off or home is not set. ' +
      'For an explicit personal metric watch ("tell me if my resting heart rate drops below 45", "tell me when I pass 10,000 steps"), set context_metric to the exact supported metric, threshold to the number the user named, and comparator to below or above. The first fresh HealthKit snapshot is only a baseline; notification requires a later real threshold crossing. Never invent a medical threshold, interpret a reading as a diagnosis, or create a health watch from a casual health question. ' +
