...
// Public Reddit JSON — no auth. Hot posts from a general news sub.
async function fetchTodayNews() {
  try {
    const resp = await axios.get('https://www.reddit.com/r/news/hot.json?limit=12', {
      headers: { 'User-Agent': 'oxy-today/1.0' }, timeout: 8000
    });
    return (resp.data?.data?.children || [])
      ...
      .map(p => ({
        ...
        source: p.domain || 'reddit'
      }));
  } catch (err) {
    console.warn('[today] news failed:', err.message);
    return [];
  }
}

// Pull a work destination string out of the user's saved memory ("works at X").
function extractWorkDestination(memoryText = '') {
  const line = String(memoryText).split('\n').find(l => /works?\s+(?:at|for)\s+/i.test(l));
  const m = line && line.match(/works?\s+(?:at|for)\s+(.+)/i);
  return m ? m[1].replace(/[.,;]+$/, '').trim() : null;
}

// Google Maps Directions with live traffic. Null unless we have a key, a current
// location, and a known work destination.
async function fetchTodayRoute(origin, destination) {
  const key = process.env.GOOGLE_MAPS_API_KEY;
  if (!key || !origin?.latitude || !origin?.longitude || !destination) return null;
  try {
    const resp = await axios.get('https://maps.googleapis.com/maps/api/directions/json', {
      params: {
        origin: `${origin.latitude},${origin.longitude}`,
        destination, departure_time: 'now', key
      },
      timeout: 8000
    });
    ...
    console.warn('[today] route failed:', err.message);
    return null;
  }
}

// Structured "Today" dashboard — real calendar/email items (with deep links to the
// native apps), live traffic to work, and a news feed. Weather stays client-side.
app.get('/today/:userId', async (req, res) => {
  if (!requireMatchingUser(req, res, req.params.userId)) return;
  const userId = req.params.userId;
  try {
    const [enabled, nativeContext, memoryText] = await Promise.all([
      getEnabledConnectors(userId).catch(() => []),
      getLatestNativeContext(userId).catch(() => null),
      getMemory(userId).catch(() => '')
    ]);
    const location = parseJsonObject(nativeContext?.location);
    const work = extractWorkDestination(memoryText);

    const calendarRun = enabled.includes('google')
      ? () => executeAction(userId, 'get_calendar_events', { max_results: 6 })
      : enabled.includes('microsoft')
        ? () => executeAction(userId, 'get_outlook_events', { max_results: 6 })
        : null;
    const emailRun = enabled.includes('google')
      ? () => executeAction(userId, 'get_emails', { max_results: 6, label: ['INBOX', 'CATEGORY_PERSONAL'] })
      : enabled.includes('microsoft')
        ? () => executeAction(userId, 'get_outlook_emails', { max_results: 6 })
        : null;

    const [calRes, mailRes, news, route] = await Promise.all([
      calendarRun ? withTimeout(calendarRun(), 12000, 'calendar').catch(() => null) : null,
      emailRun ? withTimeout(emailRun(), 12000, 'email').catch(() => null) : null,
      fetchTodayNews(),
      fetchTodayRoute(location, work)
    ]);

    const events = (calRes?.events || []).map(e => ({
      id: e.id, title: e.title || 'Untitled', start: e.start, end: e.end,
      url: e.htmlLink || null
    }));
    const emails = (mailRes?.emails || []).map(e => ({
      id: e.id,
      from: e.senderName || e.senderAddress || e.from || '',
      subject: e.subject || '(no subject)',
      snippet: e.snippet || '',
      date: e.date,
      url: e.threadId ? `https://mail.google.com/mail/u/0/#inbox/${e.threadId}` : null
    }));

    res.json({ events, emails, news, route, connectors: enabled });
  } catch (err) {
    return sendServerError(res, err, 'server.error');
  }
});
