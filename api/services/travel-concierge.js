...
function extractTravelRequirements(message = '', context = {}) {
  ...
  const requirements = {
    ...
    destination: prior.destination || extractFirst([
      // "fly/travel from X to Y" — extract Y from explicit flight/train sentences
      /\b(?:fly|travel|flying|travelling)\s+from\s+\S+(?:\s+\S+)?\s+to\s+(.+?)(?:\s+in\s+(?:jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)|\s+for\s+\d|[?.!,;]|$)/i,
      /\b(?:to|in|into|visiting|visit|for)\s+(?!fly\b|travel\b|drive\b|walk\b|take\b|get\b|go\b)(.+?)\s+(?:from|on|for|with|under|by|around|tomorrow|today|next\s+\w+|this\s+\w+)\b/i,
...
function extractActivityPreferences(lower) {
  const prefs = [];
  if (/\b(museums?|gallery|galleries|culture|cultural|history|historical|art)\b/.test(lower)) prefs.push('culture');
...
function isTravelPlanningRequest(message = '') {
  ...
  const hasTravelVocab = /\b(trip|travel|holiday|vacation|itinerary|weekend|getaway|flights?|trains?|hotel|stay|fly|flying|cruise|drive)\b/.test(lower);
  if (!hasTravelVocab) return false;
  return /\b(plan|organise|organize|book|find|build|sort|help me with|fly|travel to|going to|want to|i need|looking for)\b/.test(lower);
}
