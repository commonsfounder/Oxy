  ...
  try {
    switch (name) {
      ...
      case "get_emails": {
        ...
        console.log('GMAIL check:', { GMAIL_CLIENT_ID, GMAIL_REFRESH_TOKEN: !!GMAIL_REFRESH_TOKEN });
        if (!GMAIL_CLIENT_ID || !GMAIL_REFRESH_TOKEN) {
          return res.json({ success: false, error: "Gmail not configured" });
        }
        const token = await getGmailToken();
...
app.get("/health", (req, res) => {
  res.json({
    status: "ok",
    tools: ["send_message", "create_reminder", "get_reminders", "complete_reminder", "make_call", "play_music", "create_calendar_event", "smart_home", "send_email", "get_emails", "search_emails"],
    configured: {
